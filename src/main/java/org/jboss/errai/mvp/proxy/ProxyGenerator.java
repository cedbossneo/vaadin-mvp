...
import javassist.*;
import org.jboss.errai.mvp.annotations.ProxyEvent;
...
public class ProxyGenerator implements Serializable{
    ...
    public  <P extends Presenter<? extends View>> Class createPresenterProxy(Class<P> presenter) {
        ...
        try {
            ...
            CtConstructor constructor = CtNewConstructor.make("" +
                    "public " + presenter.getSimpleName() + "Proxy(com.google.web.bindery.event.shared.EventBus eventBus) {\n" +
                    "      super(" + presenter.getName() + ".class, eventBus);\n" +
                    "  }", proxyClass);
            registerEventsHandler(poll, presenter, proxyClass, constructor);
    ...
    private <P extends Presenter<? extends View>> void registerEventsHandler(ClassPool pool, Class<P> presenter, CtClass proxyClass, CtConstructor constructor) {
        Method[] methods = presenter.getMethods();
        for (Method method : methods) {
            if (method.isAnnotationPresent(ProxyEvent.class))
                try {
                    if (method.getParameterTypes()[0].getMethod("getType").getModifiers() == Modifier.STATIC)
                        try {
                            ...
                            proxyClass.addMethod(CtNewMethod.make(
                                    "public void "+method.getName()+"("+ event +" event){"+
                                            ...
                                            "}", proxyClass));
                            constructor.insertAfter("getEventBus().addHandler("+event+".getType(), this);");
                        } catch (NotFoundException e) {
                            e.printStackTrace();  //To change body of catch statement use File | Settings | File Templates.
                        } catch (CannotCompileException e) {
                            e.printStackTrace();  //To change body of catch statement use File | Settings | File Templates.
